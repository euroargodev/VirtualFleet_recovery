...
class RunAnalyser_core:
    ...
    def trajectory(self):
        ...
        """
        if len(self._jsdata.predictions) == 0:
        ...
        for p in self._jsdata.predictions:
...
class RunAnalyser_predictor(RunAnalyser_core):
    ...
    def fit_predict(self, weights_scale: float = 20.) -> List[Profile]:
        ...
        self._jsdata = Simulation.from_dict({
        ...
        self._add_ref()  # Fill: self._jsdata.observations
        self._predict_errors()  # Fill: self._jsdata.predictions.Metrics.error
        # self.add_metrics()
        #
        return self


class RunAnalyser_diagnostics(RunAnalyser_predictor):
    ...
    def _add_ref(self):
        ...
        This populates the ``self._jsdata.observations`` property (``self._jsdata`` was created by the ``fit_predict`` method)
        ...
        self._jsdata.observations = Plist
    ...
    def _predict_errors(self):
        ...
        This populates the ``self._jsdata.predictions.Metrics.error`` property (``self._jsdata`` was created by the ``fit_predict`` method)
        ...
        self._jsdata.predictions = Plist_updated
        return self
    ...
    def add_metrics(self, VFvel=None):
        ...
        This populates the ``self._jsdata.predictions.Metrics.transit`` and ``self._jsdata.predictions.Metrics.surface_drift`` properties (``self._jsdata`` was created by the ``fit_predict`` method)
...
class RunAnalyser_viz(RunAnalyser_diagnostics):
...
class RunAnalyser(RunAnalyser_viz):

    def to_json(self, fp=None):
        return self._jsdata.to_json(fp=fp)